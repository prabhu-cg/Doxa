...
| `STRIPE_SECRET_KEY`             | server | Billing — optional, never required locally. Without it, paid plans can't be switched to (there is no way to take payment), so an organisation stays on Free; downgrading to Free still works (`src/features/billing/checkout.ts`). |
| `STRIPE_WEBHOOK_SECRET`         | server | Signing secret for `src/app/api/webhooks/stripe/route.ts`. Without it, that route always returns 501.                                                                                                                              |
