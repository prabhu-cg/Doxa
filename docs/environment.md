...
| `STRIPE_SECRET_KEY`             | server | Billing — optional, never required locally. Without it, plan changes write directly to the local Subscription row instead of going through Stripe checkout (`src/features/billing/stripe.ts`). |
